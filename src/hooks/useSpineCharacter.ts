...
export function useSpineCharacter(
    id: string,
    { atlasPath, skeletonPath, initialScale, position = { x: 0, y: 0 } }: SpineModelOptions,
    isPlaying: boolean,
    ...
    const changeAnimation = useCallback(
        (animName: string) => {
            ...
            if (!drawableRef.current || !drawableRef.current.animationState) {
                onDebugInfo(`${id}: 드로어블 또는 애니메이션 상태가 초기화되지 않았습니다`);
                return;
            }
    ...
    const initializeSpineCharacter = async () => {
        ...
        if (!checkWebGLSupport()) {
            const errorMsg = "WebGL이 지원되지 않습니다. 브라우저를 업데이트하거나 다른 브라우저를 사용해 보세요.";
        ...
        try {
            ...
            try {
                ...
                const atlas = await window.spine.loadTextureAtlas(ck, atlasPath, readFile);
                onDebugInfo("atlas 로드 완료");

                // 스켈레톤 데이터 로드
                onDebugInfo("skeleton 데이터 로드 중...");
                const skeletonData = await window.spine.loadSkeletonData(skeletonPath, atlas, readFile);
    ...
    const startAnimationLoop = () => {
        if (!surfaceRef.current || !drawableRef.current || !rendererRef.current || !ckRef.current) {
    ...
    useEffect(() => {
        ...
        const setupCanvasKit = () => {
            try {
                if (typeof window.CanvasKitInit === "function" && typeof window.spine === "object") {
                    ...
                    setTimeout(() => {
                        if (checkCanvasKitInterval) {
                            clearInterval(checkCanvasKitInterval);
                            if (typeof window.CanvasKitInit !== "function" || typeof window.spine !== "object") {
                                const errorMsg = "CanvasKit 또는 Spine 라이브러리 로드 타임아웃";
                                onDebugInfo(errorMsg);
                                setError(errorMsg);
                            }
                        }
